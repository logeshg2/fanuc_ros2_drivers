...
"""
This is a ros2 node that read current image from the sensor and processes the image and publishes processed and center of target object.
...
class ArucoNode(Node):
    ...
    def process_image(self):
        ...
        if (ids is not None):
            ...
            aruco_corner = list(map(int, corners[0].flatten()))
            array = []
            for idx in range(0,len(aruco_corner),2):
                array.append([aruco_corner[idx], aruco_corner[idx+1]])
                self.latest_image = cv2.circle(self.latest_image, array[-1], radius=3, thickness=-1, color=(0, 255, 0))
